...
<div class="tm_pg_gallery-item col-xs-12 col-lg-<?php echo $data->size . ' ' . apply_filters( 'tm-pg-gallery-item-class', '' ) ?>" data-id="<?php echo $data->ID ?>" data-type="set">
				...
				<div class="tm_pg_gallery-item_meta">
					...
					<?php if ( $data->display['counter'][ $folder ] ): ?>
						<p class="tm_pg_gallery-item_counter"><?php
							printf(
								esc_html( _nx( '1 photo', '%1$s photos', $img_count, 'set images', 'jardinier' ) ),
								number_format_i18n( $img_count )
							);
							if ( 0 < $data->img_count['albums'] ) {
								?>, <?php
								printf(
									esc_html( _nx( '1 album', '%1$s albums', $albums_count, 'set albums', 'jardinier' ) ),
