 ...
 * @package Jardinier
 */
if ( ! class_exists( 'Jardinier_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Jardinier_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'jardinier' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'jardinier' );
			...
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => 'Image Grid',
					'label' => esc_html__( 'Widget title', 'jardinier' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category_name',
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'jardinier' ),
				...
				'tag' => array(
					...
					'label'            => esc_html__( 'Select tags to show', 'jardinier' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'jardinier' ),
						'title'         => esc_html__( 'Post Title', 'jardinier' ),
						'comment_count' => esc_html__( 'Comment Count', 'jardinier' ),
					),
					'label'   => esc_html__( 'Post sorted', 'jardinier' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'jardinier' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'jardinier' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'jardinier' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'jardinier_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'jardinier_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'jardinier_image_grid_widget_grid_class', array(
				...
				echo apply_filters(
					'jardinier_image_grid_widget_before',
				...
				foreach ( $posts as $post ) {
					...
					$image = $this->utility->media->get_image( array(
						'size'              => 'jardinier-thumb-m',
						'mobile_size'       => 'jardinier-thumb-m',
				...
				echo apply_filters( 'jardinier_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'jardinier_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function jardinier_register_image_grid_widget() {
		register_widget( 'Jardinier_Image_Grid_Widget' );
	}
}
