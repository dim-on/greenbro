...
if ( ! class_exists( 'Jardinier_Playlist_Slider_Widget' ) ) {

	/**
	 * Class Jardinier_Playlist_Slider_Widget.
	 */
	class Jardinier_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'jardinier' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'jardinier' );
			$this->widget_id          = apply_filters( 'jardinier_playlist_slider_widget_ID', 'jardinier_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'jardinier_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = jardinier_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'jardinier' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'jardinier' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'jardinier' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '525',
					'label' => esc_html__( 'Slider height ( px, rem )', 'jardinier' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'jardinier' ),
						true  => esc_html__( 'Buttons', 'jardinier' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'jardinier' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'jardinier' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'jardinier' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'jardinier' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'jardinier' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'jardinier' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'jardinier' ),
						'author'   => esc_html__( 'Author', 'jardinier' ),
						'comments' => esc_html__( 'Comment count', 'jardinier' ),
						'category' => esc_html__( 'Category', 'jardinier' ),
						'post_tag' => esc_html__( 'Tag', 'jardinier' ),
					),
					'label' => esc_html__( 'Display post meta data', 'jardinier' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'jardinier' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'jardinier_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			$thumbnails_view_dir = jardinier_get_locate_template( 'inc/widgets/playlist-slider/views/playlist-thumbnails-view.php' );
			...
			foreach ( $this->posts as $post ) {
				...
				jardinier_theme()->dynamic_css->add_style(
		...
		public function the_slides_view() {
			$slides_view_dir = jardinier_get_locate_template( 'inc/widgets/playlist-slider/views/playlist-slides-view.php' );
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'jardinier_playlist_buttons', false ),
				...
				$holder_view_dir = jardinier_get_locate_template( 'inc/widgets/playlist-slider/views/playlist-holder-view.php' );
...
add_action( 'widgets_init', 'jardinier_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function jardinier_register_playlist_slider_widget() {
	register_widget( 'Jardinier_Playlist_Slider_Widget' );
}
