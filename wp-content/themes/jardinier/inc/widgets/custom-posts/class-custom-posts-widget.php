...
if ( ! class_exists( 'Jardinier_Custom_Posts_Widget' ) ) {

	/**
	 * Class Jardinier_Custom_Posts_Widget.
	 */
	class Jardinier_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'jardinier' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'jardinier' );
			$this->widget_id          = apply_filters( 'jardinier_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'jardinier_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = jardinier_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'jardinier' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'jardinier' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'jardinier' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'jardinier' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'jardinier' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'jardinier' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'jardinier' ),
						'category'      => esc_html__( 'Category', 'jardinier' ),
						'post_tag'      => esc_html__( 'Tag', 'jardinier' ),
						'more_button'   => esc_html__( 'More Button', 'jardinier' ),
					),
				...
				'button_text' => array(
					...
					'label' => esc_html__( 'Post read more button label', 'jardinier' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', jardinier_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			if ( $posts ) {
				...
				foreach ( $posts as $post ) {
					...
					if (
						...
						$image_size = 'jardinier-thumb-480-271';
					}
	...
	add_action( 'widgets_init', 'jardinier_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function jardinier_register_custom_posts_widget() {
		register_widget( 'Jardinier_Custom_Posts_Widget' );
	}
}
