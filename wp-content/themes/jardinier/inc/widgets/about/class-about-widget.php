...
if ( ! class_exists( 'Jardinier_About_Widget' ) ) {

	/**
	 * Class Jardinier_About_Widget.
	 */
	class Jardinier_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'jardinier' );
			$this->widget_id          = 'jardinier_widget_about';
			$this->widget_name        = esc_html__( 'About Jardinier', 'jardinier' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Upload', 'jardinier' ),
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'jardinier' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'jardinier' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'jardinier' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'jardinier' ),
					'value'             => '',
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, jardinier_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			$src       = wp_get_attachment_image_src( $media_id, apply_filters( 'jardinier_about_widget_image_size', 'medium' ) );
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'jardinier_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'jardinier_register_about_widget' );
/**
 * Register about widget.
 */
function jardinier_register_about_widget() {
	register_widget( 'Jardinier_About_Widget' );
}
