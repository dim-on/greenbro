...
if ( ! class_exists( 'Jardinier_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Jardinier_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'jardinier' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'jardinier' );
			$this->widget_id          = 'jardinier_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => JARDINIER_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'layout' => array(
					...
					'label'   => esc_html__( 'Layout', 'jardinier' ),
					'options' => $layout_options,
				),
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'jardinier' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'jardinier' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'jardinier' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'jardinier' ),
						'categories' => esc_html__( 'Show categories', 'jardinier' ),
						'tags'       => esc_html__( 'Show tags', 'jardinier' ),
						'author'     => esc_html__( 'Show author', 'jardinier' ),
						'date'       => esc_html__( 'Show date', 'jardinier' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'jardinier' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'jardinier' ),
				),
			);
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = jardinier_get_locate_template( 'inc/widgets/featured-posts-block/views/item.php' );
			 ...
			 */
			$query = apply_filters( 'jardinier_featured_posts_block_query', $query, $this->instance );
		...
		public function prepare_data( $data, $layout ) {
			...
			$container_template = jardinier_get_locate_template( 'inc/widgets/featured-posts-block/views/container.php' );
		...
		public function get_layouts() {
			return array(
				'layout-1' => array(
					'name'   => esc_html__( 'Type #1', 'jardinier' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-2' => array(
					'name'   => esc_html__( 'Type #2', 'jardinier' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-3' => array(
					'name'   => esc_html__( 'Type #3', 'jardinier' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-4' => array(
					'name'   => esc_html__( 'Type #4', 'jardinier' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-5' => array(
					'name'   => esc_html__( 'Type #5', 'jardinier' ),
					'amount' => 3,
					'markup' => array(
						array(
							'type'  => 'item',
	...
	add_action( 'widgets_init', 'jardinier_register_featured_posts_block_widget' );

	if ( false === function_exists( 'jardinier_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function jardinier_register_featured_posts_block_widget() {
			register_widget( 'Jardinier_Featured_Posts_Block_Widget' );
		}
	}
}
