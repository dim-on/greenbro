 ...
 * @return array
 */
function jardinier_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! jardinier_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$defaults = apply_filters( 'jardinier_default_args_share_buttons', array(
		...
		'pinterest' => array(
			...
			'name'      => esc_html__( 'Pinterest', 'jardinier' ),
	...
	$default_config = apply_filters( 'jardinier_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'jardinier_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'jardinier' ) . $network['name'],
 ...
 * @return string
 */
function jardinier_get_single_share_buttons() {
	ob_start();
	jardinier_share_buttons( 'single' );
