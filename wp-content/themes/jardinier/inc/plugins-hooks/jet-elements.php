...
add_filter( 'jardinier_get_dynamic_css_options', 'jardinier_add_elementor_dynamic_file' );

// Add custom icon support.
add_filter( 'jet-elements/controls/icon/data', 'jardinier_set_theme_icons' );
add_action( 'elementor/editor/after_enqueue_styles', 'jardinier_enqueue_font' );
add_action( 'elementor/frontend/after_register_styles', 'jardinier_enqueue_font' );

// Add custom class at columns if paddings are used
add_action( 'elementor/frontend/element/before_render', 'jardinier_add_column_additional_classes' );

// Add custom class at tabs if border are not used
add_action( 'elementor/widget/render_content', 'jardinier_rewrite_tabs_render_template', 10, 2 );
add_action( 'elementor/widget/print_template', 'jardinier_rewrite_tabs_print_template', 10, 2 );

add_action( 'elementor/element/tabs/section_tabs/after_section_end', 'jardinier_update_tabs_controls' );

/**
 * Add builder dynamic files.
 */
function jardinier_add_elementor_dynamic_file( $options ) {

	$dynamic_files = array(
		JARDINIER_THEME_DIR . '/assets/css/dynamic/plugins/elementor.css',
	);
 ...
 * Add icon styles in admin.
 */
function jardinier_enqueue_font() {
 ...
 * Add custom icon support.
 */
function jardinier_set_theme_icons( $data ) {
	return array(
 ...
 */
function jardinier_add_column_additional_classes( \Elementor\Element_Base $element ) {
 ...
 */
function jardinier_rewrite_tabs_print_template( $template, $widget ) {
 ...
 */
function jardinier_rewrite_tabs_render_template( $element, $data ) {
	...
	<div class="<?php echo $wrap_class; ?>" role="tablist">
		...
		<div class="elementor-tabs-content-wrapper" role="tabpanel">
			<?php foreach ( $tabs as $item ) : ?>
				...
				<div class="elementor-tab-content elementor-clearfix" data-tab="<?php echo $counter; ?>"><?php echo jardinier_parse_text_editor( $item['tab_content'], $data ); ?></div>
			<?php
				$counter++;
			endforeach; ?>
		</div>
...
function jardinier_parse_text_editor( $content, $data ) {
 ...
 */
function jardinier_update_tabs_controls( $widget ) {
	...
	$widget->update_control(
		...
		array(
			'fields' => array(
				array(
					'name'        => 'tab_icon',
					'label'       => esc_html__( 'Icon', 'jardinier' ),
					'type'        => Elementor\Controls_Manager::ICON,
					'label_block' => true,
				),
				array(
					'name'        => 'tab_title',
					'label'       => esc_html__( 'Title & Content', 'jardinier' ),
					'type'        => Elementor\Controls_Manager::TEXT,
					'default'     => esc_html__( 'Tab Title', 'jardinier' ),
					'placeholder' => esc_html__( 'Tab Title', 'jardinier' ),
					'label_block' => true,
				),
				array(
					'name'        => 'tab_content',
					'label'       => esc_html__( 'Content', 'jardinier' ),
					'default'     => esc_html__( 'Tab Content', 'jardinier' ),
					'placeholder' => esc_html__( 'Tab Content', 'jardinier' ),
	...
	$widget->update_control(
		...
		array(
			'options' => [
				...
				'icon_horizontal' => esc_html__( 'Horizontal With Icon', 'jardinier' ),
				'icon_vertical'   => esc_html__( 'Vertical With Icon', 'jardinier' ),
			],
		)
	);
