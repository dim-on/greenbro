 ...
 * @return string
 */
function jardinier_get_main_menu() {
	$args = apply_filters( 'jardinier_main_menu_args', array(
		'theme_location'   => 'main',
		'container'        => '',
		'menu_id'          => 'main-menu',
		'echo'             => false,
		'fallback_cb'      => 'jardinier_set_nav_menu',
		'fallback_message' => esc_html__( 'Set main menu', 'jardinier' ),
	) );
 ...
 * @return void
 */
function jardinier_main_menu() {

	$main_menu  = jardinier_get_main_menu();
	$menu_style = get_theme_mod( 'header_menu_style', jardinier_theme()->customizer->get_default( 'header_menu_style' ) );
 ...
 * @return void
 */
function jardinier_vertical_main_menu( $slide = 'left', $args = array() ) {

	$default_args = apply_filters( 'jardinier_vertical_menu_args', array(
		...
		'button-back-inner-html'  => '<i class="nc-icon-mini arrows-1_bold-left"></i><span class="navigation-button__text">' . esc_html__( 'Back', 'jardinier' ) . '</span>',
		'button-close-inner-html' => '<i class="nc-icon-mini ui-1_bold-remove"></i><span class="navigation-button__text">' . esc_html__( 'Close', 'jardinier' ) . '</span>',
	) );

	$args        = wp_parse_args( $args, $default_args );
	$menu        = jardinier_get_main_menu();
 ...
 * @return void
 */
function jardinier_footer_menu() {
	if ( ! get_theme_mod( 'footer_menu_visibility', jardinier_theme()->customizer->get_default( 'footer_menu_visibility' ) ) ) {
		return;
	}

	$args = apply_filters( 'jardinier_footer_menu_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'jardinier_set_nav_menu',
		'fallback_message' => esc_html__( 'Set footer menu', 'jardinier' ),
	) );
 ...
 * @return void
 */
function jardinier_top_menu() {

	if ( ! jardinier_is_top_menu_visible() ) {
		return;
	}

	wp_nav_menu( apply_filters( 'jardinier_top_menu_args', array(
		'theme_location'  => 'top',
		'container'       => 'div',
 ...
 * @return bool
 */
function jardinier_is_top_menu_visible() {

	$is_visible = false;

	if ( has_nav_menu( 'top' ) && get_theme_mod( 'top_menu_visibility', jardinier_theme()->customizer->get_default( 'top_menu_visibility' ) ) ) {
		$is_visible = true;
	}
 ...
 * @return void
 */
function jardinier_login_link() {

	if ( ! jardinier_is_login_link_visible() ) {
		return;
	}
...
/**
 * Show sign up link if active.
 *
 * @since  1.0.0
 * @return void
 */
function jardinier_sign_up_link() {

    if ( ! jardinier_is_login_link_visible() ) {
        return;
    }

    do_action('cherry_popups_sign_up_link');
 ...
 * @return bool
 */
function jardinier_is_login_link_visible() {

	$is_visible = false;

	if ( has_nav_menu( 'top' ) && get_theme_mod( 'login_link_visibility', jardinier_theme()->customizer->get_default( 'login_link_visibility' ) ) ) {
		$is_visible = true;
	}
 ...
 * @return string
 */
function jardinier_get_social_list( $context, $type = 'icon' ) {
	static $instance = 0;
	$instance++;
	...
	$args = apply_filters( 'jardinier_social_list_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'jardinier_set_nav_menu',
		'fallback_message' => esc_html__( 'Set social menu', 'jardinier' ),
	), $context, $type );
 ...
 * @return null|string
 */
function jardinier_set_nav_menu( $args ) {
 ...
 * @return void
 */
function jardinier_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'jardinier_menu_toggle_html',
 ...
 * @return void
 */
function jardinier_vertical_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'jardinier_vertical_menu_toggle_html',
